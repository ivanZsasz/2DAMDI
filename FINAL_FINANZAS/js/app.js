...
class App {
    ...
    renderAccountCards(accounts) {
        ...
        const typeMap = { bank: 'Banco', cash: 'Efectivo', savings: 'Ahorro' };
    ...
    async handleSendMoney() {
        ...
        try {
            ...
            await firestoreManager.sendMoney(user, email, amount);
    ...
    handleUserAuthenticated(user) {
        ...
        if (greeting) greeting.textContent = `Hola, ${user.displayName || 'Usuario'}`;
    ...
    async toggleTransactionVisibility(id, hide) {
        if (hide) {
            this.showConfirm('¿Ocultar este movimiento? Se revertirá su impacto en el saldo.', async () => {
                await this._executeVisibilityToggle(id, hide);
            });
        } else {
            this.showConfirm('¿Mostrar este movimiento? Se volverá a aplicar al saldo.', async () => {
                await this._executeVisibilityToggle(id, hide);
            });
        }
    }
