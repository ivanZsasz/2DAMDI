...
function openServerModal(id) {
    ...
    if (!modal || !serverConfig) return;

    // Cargar datos en el modal
    const card = document.querySelector(`[data-server-id="${id}"]`);
    const serverName = card?.querySelector('.server-name')?.textContent || `Servidor ${id}`;
    const serverIP = card?.querySelector('.server-ip')?.textContent || 'N/A';
    ...
    if (statusIndicator) {
        statusIndicator.className = `modal-status-indicator ${isOnline ? 'online' : 'offline'}`;
    }

    // Inicializar gráfica del modal (más grande)
    initModalChart(id);
...
function closeServerModal(event) {
    // Si el evento viene del overlay, solo cerrar si clickeó directamente en él
...
function initModalChart(id) {
    ...
    const data = state.chartData[id] || { labels: [], ramData: [], cpuData: [] };
    ...
    modalChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: { display: true, position: 'top', labels: { color: '#8899b3', font: { size: 11 }, boxWidth: 12 } }
            },
            scales: {
                x: { display: true, grid: { color: 'rgba(136,153,179,0.1)' }, ticks: { color: '#556680', font: { size: 9 }, maxTicksLimit: 8 } },
                y: {
                    display: true,
                    min: 0,
                    max: 100,
                    grid: { color: 'rgba(136,153,179,0.1)' },
                    ticks: { color: '#556680', font: { size: 10 }, stepSize: 25, callback: v => v + '%' }
                }
            }
        }
    });
...
async function loadSchedule(id) {
    try {
        const res = await fetch(`${CONFIG.API_BASE}/api/server/${id}/schedule`);
        const data = await res.json();

        if (data.success && data.schedule) {
            document.getElementById('scheduleStart').value = data.schedule.start || '';
            document.getElementById('scheduleStop').value = data.schedule.stop || '';
        } else {
            document.getElementById('scheduleStart').value = '';
            document.getElementById('scheduleStop').value = '';
        }
    } catch (error) {
        console.log('No se pudo cargar la programación');
        document.getElementById('scheduleStart').value = '';
        document.getElementById('scheduleStop').value = '';
    }
}
...
async function saveSchedule() {
    if (!currentModalServerId) return;

    const start = document.getElementById('scheduleStart').value;
    const stop = document.getElementById('scheduleStop').value;
    const cityName = CONFIG.SERVERS[currentModalServerId].city;

    try {
        const res = await fetch(`${CONFIG.API_BASE}/api/server/${currentModalServerId}/schedule`, {
            method: 'POST',
        ...
        if (data.success) {
            showToast('success', `📅 Programación guardada para ${cityName}`);
        } else {
...
async function clearSchedule() {
    ...
    await saveSchedule(); // Guardar valores vacíos
    showToast('info', '🗑️ Programación eliminada');
}

// ==================== CLICK LISTENERS PARA TARJETAS ====================
document.addEventListener('DOMContentLoaded', () => {
    // Añadir click listener a cada tarjeta
    document.querySelectorAll('.server-card').forEach(card => {
        card.addEventListener('click', (e) => {
            // Si el click fue en un botón o dentro del footer, no abrir modal
            if (e.target.closest('.card-footer') || e.target.closest('button')) {
                return;
            }
